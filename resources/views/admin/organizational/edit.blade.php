{{-- resources/views/admin/organizational/edit.blade.php --}}
@extends('admin.layouts.app')

@section('title', 'ویرایش ' . ($organizational->getNameInLocale('fa') ?: 'غذا'))
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg">
        ...
        <div class="p-6 border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800">
                   ویرایش {{ $organizational->getNameInLocale('fa') ?: $organizational->getNameInLocale('en') ?: 'غذا' }}
                </h2>
                <a href="{{ route('admin.organizational.index') }}" class="text-gray-600 hover:text-gray-900">
        ...
        <!-- Form -->
        <form action="{{ route('admin.organizational.update', $organizational) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <!-- Category -->
            <div>
                <label for="menu_organizational_category_id" class="block text-sm font-medium text-gray-700 mb-2">دسته‌بندی *</label>
                <select name="menu_organizational_category_id" id="menu_organizational_category_id" 
                        class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 @error('menu_organizational_category_id') border-red-500 @enderror" required>
                    <option value="">انتخاب دسته‌بندی</option>
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ (old('menu_organizational_category_id', $organizational->menu_organizational_category_id) == $category->id) ? 'selected' : '' }}>
                            {{ $category->name_fa }}
                        </option>
                    @endforeach
                </select>
                @error('menu_organizational_category_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <!-- Price -->
            <div>
                <label for="price" class="block text-sm font-medium text-gray-700 mb-2">قیمت (تومان) *</label>
                <input type="number" name="price" id="price" value="{{ old('price', $organizational->price) }}" step="1" min="0"
                ...
                <p class="mt-1 text-xs text-gray-500" id="price-preview">
                    @php $currentPrice = old('price', $organizational->price); @endphp
            ...
            {{-- تصاویر فعلی + آپلود جدید --}}
            @php $currentImages = $organizational->images ?? []; @endphp
            ...
            <div class="flex justify-end space-x-3 rtl:space-x-reverse">
                <a href="{{ route('admin.organizational.index') }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">انصراف</a>
