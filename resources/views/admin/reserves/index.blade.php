                ...
                <div class="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    ...
                    <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                        <div>
                            ...
                            <p class="text-gray-900 dir-ltr" dir="ltr">{{ $reserve->phone }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500 text-xs">ایمیل:</span>
                            <p class="text-gray-900 truncate">{{ $reserve->email ?? '-' }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500 text-xs">نوع مراسم:</span>
                            <p class="text-gray-900">{{ $reserve->event_type ?? '-' }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500 text-xs">تعداد مهمان:</span>
                            <p class="text-gray-900">{{ $reserve->guest_count ?? '-' }} نفر</p>
                        </div>
                        <div>
                            <span class="text-gray-500 text-xs">تاریخ رزرو:</span>
                            <p class="text-gray-900">{{ $reserve->reservation_date }}</p>
                        </div>
                        <div>
                            <span class="text-gray-500 text-xs">ساعت:</span>
                            <p class="text-gray-900">{{ $reserve->entry_time }} تا {{ $reserve->exit_time }}</p>
                        </div>
                        <div class="col-span-2">
                            <span class="text-gray-500 text-xs">تاریخ ثبت:</span>
                            <p class="text-gray-900">{{ $reserve->created_at->format('Y/m/d H:i') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                        <a href="{{ route('admin.reserves.show', $reserve->id) }}" 
                           class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-md text-white text-xs font-medium bg-sky-500 hover:bg-sky-600 transition-colors">
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-right font-semibold text-gray-700">نام و نام خانوادگی</th>
                            ...
                            <th class="px-4 py-3 text-right font-semibold text-gray-700">تعداد مهمان</th>
                            <th class="px-4 py-3 text-right font-semibold text-gray-700">تاریخ رزرو</th>
                            <th class="px-4 py-3 text-right font-semibold text-gray-700">ساعت</th>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3 text-gray-600">{{ $reserve->email ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $reserve->event_type ?? '-' }}</td>
                            <td class="px-4 py-3 text-gray-600">{{ $reserve->guest_count ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3">
                                @if($reserve->status === 'pending')
                                    <span class="inline-flex items-center rounded-full bg-yellow-100 px-2.5 py-0.5 text-xs font-medium text-yellow-800">در انتظار بررسی</span>
                                @elseif($reserve->status === 'confirmed')
                                    <span class="inline-flex items-center rounded-full bg-green-100 px-2.5 py-0.5 text-xs font-medium text-green-800">تأیید شده</span>
                                @elseif($reserve->status === 'rejected')
                                    <span class="inline-flex items-center rounded-full bg-red-100 px-2.5 py-0.5 text-xs font-medium text-red-800">رد شده</span>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('admin.reserves.show', $reserve->id) }}" 
                                       class="inline-flex items-center justify-center p-1.5 rounded-md text-white bg-sky-500 hover:bg-sky-600 transition-colors" 
                                       title="مشاهده جزئیات">
                                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <form action="{{ route('admin.reserves.destroy', $reserve->id) }}" 
                                        ...
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center justify-center p-1.5 rounded-md text-white bg-red-500 hover:bg-red-600 transition-colors" title="حذف">
                                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                        </button>
                                    </form>
                                </div>
