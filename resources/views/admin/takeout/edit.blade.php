{{-- resources/views/admin/takeout/edit.blade.php --}}
@extends('admin.layouts.app')

@section('title', 'ویرایش ' . ($takeout->getNameInLocale('fa') ?: 'غذا'))
...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg">
        ...
        <div class="p-6 border-b border-gray-200">
            <div class="flex justify-between items-center">
                <h2 class="text-2xl font-bold text-gray-800">
                   ویرایش {{ $takeout->getNameInLocale('fa') ?: $takeout->getNameInLocale('en') ?: 'غذا' }}
                </h2>
                <a href="{{ route('admin.takeout.index') }}" class="text-gray-600 hover:text-gray-900">
        ...
        <!-- Form -->
        <form action="{{ route('admin.takeout.update', $takeout) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="menu_takeout_category_id" id="menu_takeout_category_id" 
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ (old('menu_takeout_category_id', $takeout->menu_takeout_category_id) == $category->id) ? 'selected' : '' }}>
                            {{ $category->name_fa }}
                        </option>
                    @endforeach
                </select>
                @error('menu_takeout_category_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="price" class="block text-sm font-medium text-gray-700 mb-2">قیمت (تومان) *</label>
                <input type="number" name="price" id="price" value="{{ old('price', $takeout->price) }}" step="1" min="0"
                ...
                <p class="mt-1 text-xs text-gray-500" id="price-preview">
                    @php $currentPrice = old('price', $takeout->price); @endphp
            ...
            @php $currentImages = $takeout->images ?? []; @endphp
            ...
            <div class="flex justify-end space-x-3 rtl:space-x-reverse">
                <a href="{{ route('admin.takeout.index') }}" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors">انصراف</a>
