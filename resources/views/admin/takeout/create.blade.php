...
<div class="max-w-3xl mx-auto">
    <div class="bg-white rounded-lg shadow-lg">
        ...
        <!-- Form -->
        <form action="{{ route('admin.takeout.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select name="menu_takeout_category_id" id="menu_takeout_category_id" 
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('menu_takeout_category_id') == $category->id ? 'selected' : '' }}>
                            {{ $category->name_fa }}
                        </option>
                    @endforeach
                </select>
                ...
                @if($categories->isEmpty())
                    <p class="mt-2 text-sm text-yellow-600">
                        ...
                        <a href="{{ route('admin.takeout-categories.create') }}" class="underline text-rose-500">یک دسته جدید بسازید</a>.
                    </p>
                @endif
            </div>
